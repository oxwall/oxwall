    ...
    public function render()
    {
        ...
        foreach ( $this->meta as $key => $value )
        {
            if ( in_array($key, $this->availableMetaAttrs) && !empty($value) )
            {
                foreach ( $value as $name => $content )
                {
                    ...
                    $headData .= HtmlTag::generateTag("meta", $attrs) . PHP_EOL;
                }
            }
        }
        ...
        foreach ( $this->styleSheets["items"] as $priority => $scipts )
        {
            foreach ( $scipts as $media => $urls )
            {
                foreach ( $urls as $url )
                {
                    ...
                    $headData .= HtmlTag::generateTag("link", $attrs) . PHP_EOL;
                }
            }
        }
        ...
        foreach ( $this->preIncludeJavaScriptDeclarations as $priority => $types )
        {
            foreach ( $types as $type => $declarations )
            {
                foreach ( $declarations as $declaration )
                {
                    ...
                    $jsData .= HtmlTag::generateTag("script", $attrs, true, PHP_EOL . $declaration . PHP_EOL) . PHP_EOL;
                }
            }
        }
        ...
        foreach ( $this->javaScripts["items"] as $priority => $types )
        {
            foreach ( $types as $type => $urls )
            {
                foreach ( $urls as $url )
                {
                    ...
                    if ( $priority == -100 )
                    {
                        $headJsInclude .= HtmlTag::generateTag("script", $attrs, true) . PHP_EOL;
                    }
                    else
                    {
                        $jsData .= HtmlTag::generateTag("script", $attrs, true) . PHP_EOL;
                    }
                }
            }
        }
        ...
        foreach ( $this->styleDeclarations["items"] as $priority => $mediaTypes )
        {
            foreach ( $mediaTypes as $media => $declarations )
            {
                ...
                $headData .= HtmlTag::generateTag("style", $attrs, true, implode(" ", $declarations));
            }
        }
        ...
        foreach ( $this->javaScriptDeclarations["items"] as $priority => $types )
        {
            foreach ( $types as $type => $declarations )
            {
                ...
                {
                    $attrs = array("type" => $type);
                    $jsData .= HtmlTag::generateTag("script", $attrs, true,
        ...
        {
            $headData .= HtmlTag::generateTag("link", $linkInfo) . PHP_EOL;
        }
        ...
        $assignArray = array(
            ...
            "pageBody" => $this->prependCode . $masterPageOutput . $this->appendCode . $jsData . Document::APPEND_PLACEHOLDER,
