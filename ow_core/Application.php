...
class Application
{
    ...
    public function init()
    {
        ...
        Auth::getInstance()->setAuthenticator(new SessionAuthenticator());
        $this->userAutoLogin();
        ...
        $event = new \OW_Event(EventManager::ON_PLUGINS_INIT);
        ...
        {
            ThemeManager::getInstance()->setCurrentTheme(\BOL_ThemeService::getInstance()->getThemeObjectByKey(trim($activeThemeName)));
        }
        ...
        {
            OW::getRouter()->addRoute(new Route($value->getKey(), $value->getUri(),
                $staticPageDispatchAttrs["controller"], $staticPageDispatchAttrs["action"],
                array("documentKey" => array(Route::PARAM_OPTION_HIDDEN_VAR => $value->getKey()))));

            // TODO refactor - hotfix for TOS page
            if ( in_array(\Oxwall\Utilities\String::removeFirstAndLastSlashes($value->getUri()),
        ...
        if ( $item !== null )
        {
            ...
            $router->addRoute(new Route("base_default_index", "/", $ddispatchAttrs["controller"],
                $ddispatchAttrs["action"],
                array("documentKey" => array(Route::PARAM_OPTION_HIDDEN_VAR => $item->getDocumentKey()))));
            $this->indexMenuItem = $item;
            OW::getEventManager()->bind(EventManager::ON_AFTER_REQUEST_HANDLE, array($this, "activateMenuItem"));
        }
        else
        {
            $router->addRoute(new Route("base_default_index", "/", "BASE_CTRL_ComponentPanel", "index"));
        }

        if ( !OW::getRequest()->isAjax() )
        {
            OW::getResponse()->setDocument($this->newDocument());
            OW::getDocument()->setMasterPage(new MasterPage());
            OW::getResponse()->setHeader(Response::HD_CNT_TYPE,
                OW::getDocument()->getMime() . "; charset=" . OW::getDocument()->getCharset());
        }
        else
        {
            OW::getResponse()->setDocument(new AjaxDocument());
        }
    ...
    public function handleRequest()
    {
        ...
        {
            $attributes = array(
                RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => "BASE_CTRL_User",
                RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => "standardSignIn"
            );
        ...
        {
            $attributes = array(
                RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => "BASE_CTRL_BaseDocument",
                RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => "splashScreen",
                RequestHandler::CATCH_ALL_REQUEST_KEY_REDIRECT => true,
                RequestHandler::CATCH_ALL_REQUEST_KEY_JS => true,
                RequestHandler::CATCH_ALL_REQUEST_KEY_ROUTE => "base_page_splash_screen"
            );
        ...
        )
        {
            $attributes = array(
                RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => "BASE_CTRL_BaseDocument",
                RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => "passwordProtection"
            );
        ...
        if ( (bool) $baseConfigs["maintenance"] && !OW::getUser()->isAdmin() )
        {
            $attributes = array(
                RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => "BASE_CTRL_BaseDocument",
                RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => "maintenance",
                RequestHandler::CATCH_ALL_REQUEST_KEY_REDIRECT => true
            );
    ...
    public function finalize()
    {
        ...
        $event = new \OW_Event(EventManager::ON_FINALIZE);
        OW::getEventManager()->trigger($event);
    }
    ...
    public function redirect( $redirectTo = null, $switchContextTo = false )
    {
        ...
        {
            $redirectTo = OW::getRouter()->getBaseUrl() . \Oxwall\Utilities\String::removeFirstAndLastSlashes($redirectTo);
        }

        \Oxwall\Utilities\Url::redirect($redirectTo);
    }
    /**
    ...
    public function activateMenuItem()
    {
        if ( !OW::getDocument()->getMasterPage() instanceof \ADMIN_CLASS_MasterPage )
        {
            ...
            {
                OW::getNavigation()->activateMenuItem(Navigation::MAIN, $this->indexMenuItem->getPrefix(),
    ...
    protected function newDocument()
    {
        ...
        OW::getEventManager()->bind(EventManager::ON_AFTER_REQUEST_HANDLE, array($this, "onBeforeDocumentRender"));
...
//        }
        ...
        if ( OW_PROFILER_ENABLE )
        {
            ...
            function base_dev_tool( \BASE_CLASS_EventCollector $event )
            {
                ...
                $viewRenderer->assignVar("profiler", \Oxwall\Utilities\Profiler::getInstance()->getResult());
                ...
                $rndItems = Renderable::getRenderedClasses();
                ...
                $viewRenderer->assignVar('renderedItems',
                    array('items' => $rndArray, 'count' => ( count(Renderable::getRenderedClasses()) - 2 )));
        ...
        {
            ViewRenderer::getInstance()->clearCompiledTpl();
        }
    ...
    protected function updateCachedEntities( $options )
    {
        ...
        {
            OW::getCacheManager()->clean(array(), CacheManager::CLEAN_ALL);
        }
    ...
    {
        $this->httpsHandlerAttrsList[] = array(RequestHandler::ATTRS_KEY_CTRL => $controller, RequestHandler::ATTRS_KEY_ACTION => $action);
    }
    ...
    protected function httpVsHttpsRedirect()
    {
        ...
        {
            if ( $item[RequestHandler::ATTRS_KEY_CTRL] == $attrs[RequestHandler::ATTRS_KEY_CTRL] && ( empty($item[RequestHandler::ATTRS_KEY_ACTION]) || $item[RequestHandler::ATTRS_KEY_ACTION] == $attrs[RequestHandler::ATTRS_KEY_ACTION] ) )
            {
                $specAttrs = true;
                if ( !$isSsl )
                {
    ...
    protected function handleHttps()
    {
        ...
        OW::getEventManager()->bind(EventManager::ON_AFTER_DOCUMENT_RENDER, "base_post_handle_https_static_content");
    }
