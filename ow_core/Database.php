...
class Database
{
    ...
    private function __construct( $params )
    {
        ...
        try
        {
            ...
            if ( !empty($params["profilerEnable"]) )
            {
                ...
                $this->profiler = \Oxwall\Utilities\Profiler::getInstance("db");
                $this->queryCount = 0;
    ...
    public static function getInstance( $params )
    {
        ...
        if ( empty(self::$classInstances[$connectionKey]) )
        {
            ...
            {
                throw new \InvalidArgumentException("Cant connect to database. Please provide valid connection attributes.");
            }
    ...
    public function insertObject( $tableName, $obj, $delayed = false )
    {
        if ( $obj != null && is_object($obj) )
        {
            ...
            $columns = \Oxwall\Utilities\String::arrayToDelimitedString($paramNames, ",", "`", "`");
            $values = \Oxwall\Utilities\String::arrayToDelimitedString($paramNames, ",", ":");
    ...
    public function updateObject( $tableName, $obj, $primaryKeyName = "id", $lowPriority = false )
    {
        if ( $obj != null && is_object($obj) )
        {
            ...
            $updateStmt = \Oxwall\Utilities\String::arrayToDelimitedString($updateArray);
