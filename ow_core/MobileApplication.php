...
//        }
        ...
        {
            ThemeManager::getInstance()->setCurrentTheme(\BOL_ThemeService::getInstance()->getThemeObjectByKey(trim($activeThemeName), true));
        }
        ...
        {
            OW::getRouter()->addRoute(new Route($value->getKey(), $value->getUri(), $staticPageDispatchAttrs["controller"], $staticPageDispatchAttrs["action"], array("documentKey" => array(Route::PARAM_OPTION_HIDDEN_VAR => $value->getKey()))));

            // TODO refactor - hotfix for TOS page
            if ( \Oxwall\Utilities\String::removeFirstAndLastSlashes($value->getUri()) == "terms-of-use" )
            {
        ...
        $item = \BOL_NavigationService::getInstance()->findFirstLocal((OW::getUser()->isAuthenticated() ? \BOL_NavigationService::VISIBLE_FOR_MEMBER : \BOL_NavigationService::VISIBLE_FOR_GUEST), Navigation::MOBILE_TOP);
        ...
        if ( $item !== null )
        {
            ...
            $router->addRoute(new Route("base_default_index", "/", $ddispatchAttrs["controller"], $ddispatchAttrs["action"], array("documentKey" => array(Route::PARAM_OPTION_HIDDEN_VAR => $item->getDocumentKey()))));
            $this->indexMenuItem = $item;
            OW::getEventManager()->bind(EventManager::ON_AFTER_REQUEST_HANDLE, array($this, "activateMenuItem"));
        }
        else
        {
            $router->addRoute(new Route("base_default_index", "/", "BASE_MCTRL_WidgetPanel", "index"));
        }

        if ( !OW::getRequest()->isAjax() )
        {
            OW::getResponse()->setDocument($this->newDocument());
            OW::getDocument()->setMasterPage(new MobileMasterPage());
            OW::getResponse()->setHeader(Response::HD_CNT_TYPE, OW::getDocument()->getMime() . "; charset=" . OW::getDocument()->getCharset());
        }
        else
        {
    ...
    public function handleRequest()
    {
        ...
        {
            $attributes = array(
                RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => "BASE_MCTRL_User",
        ...
        {
            $attributes = array(
                RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => "BASE_MCTRL_BaseDocument",
