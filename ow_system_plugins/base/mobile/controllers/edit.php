...
class BASE_MCTRL_Edit extends BASE_CTRL_Edit
{
    ...
    public function index( $params )
    {
        ...
        // add avatar field
        $avatarValidator = OW::getClassInstance("BASE_MCLASS_EditAvatarFieldValidator", false);
        ...
        if ($displayPhotoUpload == BOL_UserService::CONFIG_JOIN_DISPLAY_AND_SET_REQUIRED_PHOTO_UPLOAD) {
            $avatarValidator = OW::getClassInstance("BASE_MCLASS_EditAvatarFieldValidator", true, $userId);
        }
        ...
        $this->setTemplate(OW::getPluginManager()->getPlugin('base')->getMobileCtrlViewDir() . 'edit_index.html');
    }
        ...
        if ( $editForm->isValid($_POST) )
        {
            ...
            if ( !empty($userId) )
            {
                ...
                if ( empty($_POST['userPhoto']) )
                {
                    if ( $displayPhotoUpload == BOL_UserService::CONFIG_JOIN_DISPLAY_AND_SET_REQUIRED_PHOTO_UPLOAD )
                    {
                        OW::getFeedback()->error($language->text('mobile', 'avatar_cannot_be_deleted'));
                ...
                if ( !empty($_FILES['userPhoto']['tmp_name']) )
                {
                    ...
                    {
                        OW::getFeedback()->error($language->text('mobile', 'wrong_avatar_format'));
