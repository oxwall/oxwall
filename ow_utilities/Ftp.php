...
class Ftp
{
    ...
    public function rmDir( $dirPath )
    {
        ...
        $dirPath = File::removeLastDS($dirPath);
    ...
    public function mkDir( $dirPath )
    {
        ...
        if ( $result === false )
        {
            trigger_error("Cant create dir by FTP `{$dirPath}`", E_USER_WARNING);
        }
    }
    ...
    public function readDir( $dirPath, $type = "all" )
    {
        ...
        {
            trigger_error("Cant set passive mode for FTP connection", E_USER_WARNING);
        }
        ...
        if ( $list === false )
        {
            trigger_error("Cant get dir contents `{$dirPath}`", E_USER_WARNING);
            return array();
        }
    ...
    public function chmod( $mode, $filePath )
    {
        ...
        if ( $result === false )
        {
            throw new \LogicException("Cant chmod by FTP `{$filePath}`");
        }
    }
    ...
    public function upload( $localFile, $remoteFile )
    {
        ...
        {
            trigger_error("Cant upload file by FTP `{$localFile}`", E_USER_WARNING);
        }
    }
    ...
    public function uploadDir( $localDir, $remoteDir )
    {
        ...
        {
            trigger_error("Cant read dir `{$localDir}`!", E_USER_WARNING);
            return;
        }
    ...
    public function download( $remoteFile, $localFile )
    {
        ...
        {
            throw new \LogicException("Cant download file by FTP `{$remoteFile}`");
        }
    }
